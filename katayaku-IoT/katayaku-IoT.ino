...
const int pinMQ = 34;   // Pin DO on the DOIT ESP32 DevKit
MQ135 gasSensor = MQ135(pinMQ);

// Define the pin connected to the relay
const int relayPin = 25;  // Change this to the GPIO pin you connected the relay's signal input to

// Instancias de la clase Motores
Motores motor1(21, 19, 22);  // IN1, IN2, PWM1
Motores motor2(18, 5, 4);  // IN3, IN4, PWM2
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  else if (String(topic) == "motores/uno") {
    motor1.controlarPorMQTT(message);
  }else if (String(topic) == "motores/dos") {
    motor2.controlarPorMQTT(message);
  }
}
...
void setup() {
  ...
  digitalWrite(relayPin, HIGH); // Initialize the relay in the off state
...
void loop() {
  if (!client.connected() || WiFi.status() != WL_CONNECTED) {
    ...
    String mensaje = "Gas: " + String(ppm);

    client.publish("sensors/gas", mensaje.c_str());
    Serial.println("Sensor value: " + String(valorSensor) +
